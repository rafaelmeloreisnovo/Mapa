...
The scanner is evidence-first and privacy-preserving:
- reads JSON/JSONL (optionally gzip) recursively;
- never emits source text;
- records only hashes/pointers, matched canonical terms, structural signals,
  co-occurrence edges and auditable receipts;
- never promotes a match into runtime/build/claim evidence.
"""
...
def normalize(text: str) -> str:
    ...
    return ascii_text.casefold().replace("_", " ")
...
def compile_rules(rules_path: Path):
    ...
    for rule in data.get("rules", []):
        named.append((rule, [re.compile(pattern, re.IGNORECASE) for pattern in rule["patterns"]]))
    signals = []
    for signal in data.get("structural_signals", []):
        signals.append((signal, [re.compile(pattern, re.IGNORECASE) for pattern in signal["patterns"]]))
...
def scan(inputs: list[Path], rules_path: Path, output_dir: Path, fail_on_parse_error: bool = False) -> dict[str, Any]:
    ...
    relation_sources: dict[tuple[str, str], set[str]] = defaultdict(set)
                ...
                for json_pointer, raw_text in iter_strings(record):
                    ...
                    for rule, patterns in named_rules:
                        ...
                        row = {
                            ...
                            "record_pointer_hash": sha256_bytes(record_pointer.encode("utf-8"))[:24],
                            "json_pointer_hash": sha256_bytes(json_pointer.encode("utf-8"))[:24],
                        ...
                        matched_here.append(row)

                    canonical_names = sorted({row["canonical"] for row in matched_here})
                    for i, left in enumerate(canonical_names):
                        for right in canonical_names[i + 1 :]:
                            pair = canonical_pair(left, right)
                            relation_counts[pair] += 1
                            relation_sources[pair].add(source_id)
        except Exception as exc:  # parse boundary is captured, never silently swallowed
            file_ok = False
            parse_errors.append({
                "source_id": source_id,
    ...
    candidates.sort(key=lambda row: (row["canonical"], row["source_id"], row["location_id"], row["rule_id"]))
    relations = []
    for (left, right), count in sorted(relation_counts.items()):
        relations.append({
            ...
            "left": left,
            "right": right,
            "relation": "CO_OCCURS_IN_SOURCE_LOCATION",
            "occurrences": count,
            "source_count": len(relation_sources[(left, right)]),
            "state": "OBSERVED_COOCCURRENCE_NOT_CAUSATION",
            "claim_allowed": False,
        })

    by_canonical = Counter(row["canonical"] for row in candidates)
    by_kind = Counter(row["kind_hint"] for row in candidates)
    by_state = Counter(row["state"] for row in candidates)
    known = sum(1 for row in candidates if row["architecture_id"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        scan(args.inputs, args.rules, args.output_dir, args.fail_on_parse_error)
