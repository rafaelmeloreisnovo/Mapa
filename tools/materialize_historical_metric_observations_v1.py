...
claim_allowed=false. Supersession, negative results, source hashes and open gaps
remain explicit.
"""
...
def normalize(value, unit):
    if unit in UNIT_RULES:
        ...
        return {"status":"NORMALIZED","canonical_value":value*factor,"canonical_unit":canonical_unit,
                "dimension":dimension,"rule_id":rule_id}
    return {"status":"PRESERVED_SEMANTIC","canonical_value":value,"canonical_unit":unit,
            "dimension":"source_semantic","rule_id":"PRESERVE_SOURCE_SEMANTIC_V1"}
...
def materialize(seed):
    out=[]; sources=seed["sources"]
    for run in seed["runs"]:
        src=sources[run["source"]]
        source_ref={"provider_id":src["provider_id"],"file":f"conversations-{run['source']}.json",
                    "conversation_id":run["conversation_id"]}
        if run.get("message_id"): source_ref["message_id"]=run["message_id"]
        supersession=run.get("supersession",{}); run_open=list(run.get("open_tokens",[]))
        for symbol,value,unit,category in run["metrics"]:
            oid=metric_id(run["run_id"],symbol)
            record={
                "schema":"RAFAELIA_METRIC_OBSERVATION_V1","observation_id":oid,
                "metric_name":f"{run['run_id']} {symbol}","category":category,"value_state":"HISTORICAL",
                "epistemic_class":"HISTORICAL_RUN_OUTPUT","observed_value":value,"original_unit":unit,
                "normalization":normalize(value,unit),"statistic":{"kind":"UNKNOWN","sample_count":None,"repeats":None,"confidence":None},
                "direction":"CONTEXT_DEPENDENT","scope":f"historical shard {run['source']} / {run['run_id']}",
                "source_refs":[source_ref],"config":run.get("config",{}),
                "evidence":{"artifact_sha256":src["sha256"],"receipt_refs":[],
                            "raw_result_refs":[src["provider_id"],run["conversation_id"]]+([run["message_id"]] if run.get("message_id") else []),
                            "digest_verified":True},
                "negative_evidence":symbol.startswith("UNROLL4") or symbol.startswith("UNROLL8"),
                ...
                "claim_allowed":False,"notes":run.get("notes",""),
...
def main():
    ...
    args=ap.parse_args()
    seed=json.loads(Path(args.seed).read_text(encoding="utf-8")); records=materialize(seed)
    ids=[r["observation_id"] for r in records]
    if len(ids)!=len(set(ids)): raise SystemExit("duplicate observation_id")
