...
Every new reversible federated operation SHOULD emit or reference:

1. stable `event_id`;
2. producer repository and exact ref/SHA;
3. `before_ref`;
4. `after_ref`;
5. `rollback_ref`;
6. `evidence_ref`;
7. predecessor event when applicable;
8. `reversibility_state`;
9. `risk_class`;
10. `claim_allowed=false` until an independent promotion gate exists.
...
Legacy receipt without a `reversibility` block:

`LEGACY_RECEIPT_PRESENT + REVERSIBILITY_METADATA=TOKEN_VAZIO`

not:

`LEGACY_RECEIPT_IRREVERSIBLE`

and not:

`REVERSIBILITY_VERIFIED`.

Migration is therefore monotonic: add a successor receipt/event or an external pointer; never rewrite the historical receipt just to satisfy the new schema.

## Current operational boundaries

The repository-local broker currently states that remote receipt transport is not implemented, production HMAC verification is not implemented, and external federation remains `TOKEN_VAZIO`. Documentation or examples that describe those surfaces as already deployed production services must be interpreted as design/onboarding intent until provider-bound execution evidence exists.
