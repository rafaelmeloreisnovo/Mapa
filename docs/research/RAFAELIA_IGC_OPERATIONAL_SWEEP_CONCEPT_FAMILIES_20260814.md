...
Apply the already-materialized **Invariante Geométrica Coerente / Coesão Real (IGC-CR)** as an operational filter over the concept-anteriority families recorded in PR #231.
...
## 1. Operational classes

### A. DIRECT_GEOMETRIC
Objects whose ordinary semantics are explicitly geometric/topological.
...
Objects whose useful invariant is structural/byte/combinatorial rather than Euclidean geometry.
...
Candidate invariants include exact roundtrip bytes, archive compatibility, record cardinality/layout, declared graph/incidence relations, digest/signature verification and fail-closed rejection behavior. Cryptographic authenticity is not a geometric invariant and must remain a separate trust layer.

### C. STATE_SPACE_OPERATIONAL
Objects that may admit a graph/state-space geometry only if states and transition operators are explicitly defined.
...
No family in this class receives a geometric claim from its name alone. Minimum declaration is `state set + transition operator + equivalence relation + observable invariant`.

### D. EMPIRICAL_BIOPHYSICAL
Objects require measurement models before any invariant can be tested.

- BIOSSINTETICO
- BIOPHOTON/BIOFOTON

Possible graph/topology/state-space models are hypotheses only until dataset, measurement procedure, uncertainty and falsifier are bound.

### E. META_MATHEMATICAL
- CLAYMATH

This is not one geometry. Each Millennium-problem-related construction must inherit the transformation/invariant language of the specific mathematical problem. Discussion or internal derivation is not a solved-problem claim.
...
## 3. Strongest currently defensible cross-family relations

### REL-IGC-ZIPRAF-BITRAF
`ZIPRAF -> structural layer -> BITRAF` is a meaningful combinatorial relation **when versioned artifacts prove the linkage**. It is not evidence that BITRAF itself is Euclidean geometry.

### REL-IGC-HYPERCUBE-TESSERACT
`TESSERACT` is a specific 4D hypercube model. RAFAELIA-specific novelty, if any, must attach to encoding/projection/state/algorithm, not to the mathematical object itself.

### REL-IGC-STATE-LAYERS
`RafMem/RafStore/NetRaf/RAFCODE/RafaelOS` can share one abstract state-space method only after each publishes its own state set and transition operators. Shared naming ecosystem is not shared geometry.
...
## 4. Anti-regression falsifiers

The sweep must fail closed if any of the following occurs:
...
6. `state graph analogy => topology proven`.
...
## 5. Operational excellence lane

For every family F, maintain this ordered lane:
...
- Current control plane: Mapa PR #231 / `audit/concept-anteriority-batch-20260813`

## 7. F_OK / F_GAP / F_NEXT

### F_OK
- IGC-CR is already materialized and historically merged.
- Positive and negative local geometry fixtures were reproduced in the prior receipt.
- Concept-family anteriority registry exists and is append-only.
