...
state: DERIVED
evidence: "arquivo novo em branch isolada + PR draft #381"
...
mitigation: "novo caminho + branch isolada + PR + claim_allowed=false + alinhamento explícito à cadeia de custódia e ao workflow TOKEN_VAZIO"
...
mitigation: "G0..G6 deste ledger rebaixados a pré-gate de ingestão; fechamento continua no workflow canônico."
gap: TOKEN_VAZIO
...
gap: "registro efetivo destes eventos no JSONL canônico ainda não realizado neste PR"
...
Correção aplicada: os gates `G0..G6` deixaram de ser descritos como gate final e passaram a ser pré-gate de ingestão. O receipt ad hoc foi substituído por referência ao contrato canônico de cadeia de custódia.
...
- [x] diff do PR inspecionado: somente este novo arquivo
